...
**⚠️ Hallazgo grave, pre-existente — editar Letras borraba los acordes:** el editor de Letras arranca con `plainLyrics(song.chordpro)` (la letra **sin acordes**) y al Guardar hace `updateSong({ chordpro: draft })`: **cualquier guardado desde Letras reemplaza el chordpro completo por la letra sin acordes** — se pierden acordes, notas y marcas de toda la canción, sin forma de recuperarlos. Verificado leyendo el código (`LetrasPage.tsx`, `handleStartEditing`/`handleSaveLyrics`); en la base local las 22 canciones todavía tienen acordes.

**Resuelto — decisión de Pablo: editar solo desde Acordes.** Se eliminó el editor de Letras (borrador, atajos de sección, guardado) y las funciones que armaban la "letra sin acordes" para editar (`lyricsLines`/`plainLyrics`, sin otros usos). El botón **Editar** de Letras navega a `/acordes?songId=…&editar=true`, que abre directamente el editor de Acordes (canción completa + vista previa en vivo); la ruta `/acordes` acepta el nuevo parámetro opcional `editar` y lo aplica una sola vez, cuando la canción pedida ya cargó. Verificado con navegador real, sin guardar: Letras → Editar abre el editor de Acordes con acordes, notas y vista previa; Cancelar lo cierra.
...
- `ChordSheet`: en "Letra + acordes" la nota va **siempre en la fila de los acordes**, del **mismo color que los acordes** y **sin cortar la letra** (dos correcciones de Pablo sobre versiones anteriores: primero iba en la fila de la letra, después ocupaba lugar y dejaba un hueco en la letra). Se dibuja encima de la fila sin ocupar ancho y **la letra nunca se mueve** (tercera corrección de Pablo: la versión anterior corría la columna del acorde siguiente y dejaba un hueco en la letra). `placeNotes`, en caracteres (la fuente es monoespaciada): la nota va donde se escribió; si no entra antes del acorde siguiente, se corre a la izquierda al espacio libre de la fila de acordes; solo si tampoco entra así, se corre **el acorde** (con `left` relativo, sin afectar la letra), en cadena para que los acordes siguientes tampoco se superpongan; en "Solo acordes" el texto de compases se arma por segmentos (`chordChartSegments`) y la nota corta el texto donde se escribió (`| F - ↱ coro suave C | Am |:]`).
- PDF: mismo criterio en dos pasadas (primero letra y columnas de acordes sin notas, después ubicación de notas y corrimiento de acordes) — la letra nunca se mueve.
