...
- `ChordSheet`: en "Letra + acordes" la nota va **siempre en la fila de los acordes**, del **mismo color que los acordes** y **sin cortar la letra** (dos correcciones de Pablo sobre versiones anteriores: primero iba en la fila de la letra, después ocupaba lugar y dejaba un hueco en la letra). Se dibuja encima de la fila sin ocupar ancho; solo si el acorde siguiente quedaría debajo de la nota (`noteShifts`, en caracteres — la fuente es monoespaciada) se corre ese acorde lo justo, y solo en ese caso aparece un hueco mínimo en la letra; en "Solo acordes" el texto de compases se arma por segmentos (`chordChartSegments`) y la nota corta el texto donde se escribió (`| F - ↱ coro suave C | Am |:]`).
- PDF: mismo criterio — la nota va en la fila de los acordes, dorada y más chica, sin reservar lugar; el acorde siguiente se corre solo si quedaría debajo de la nota.
